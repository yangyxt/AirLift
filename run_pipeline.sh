...
function extract_reads_with_parallel(){
    ...
    for i in `ls ${OUTPUT}/*.chain`; do 
        ...
        samtools view -H -) <(/usr/bin/time -v -p -o "${OUTPUT}/bedfiles/${chr}/constant_bed/constant_liftOver.time" liftOver -minMatch=1 <(samtools view -h -L "${OUTPUT}/bedfiles/${chain_basename}/constant_bed/constant_regions.bed" ${READ_BAM} | bamToBed -i -) ${i} >("${SRCDIR}/5-merge/liftBedToSam" <(samtools view -L "${OUTPUT}/bedfiles/${chain_basename}/constant_bed/constant_regions.bed" ${READ_BAM}) - 4 3,4 1,2) "${OUTPUT}/bedfiles/${chain_basename}/constant_bed/constant_unmapped.bed") | \
